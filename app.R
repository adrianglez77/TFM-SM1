...
server <- function(input, output, session) {
 ...
 # Generar la selección de partidos según el equipo seleccionado
 output$partidos_select <- renderUI({
   ...
   # Obtener los oponentes para los partidos del equipo seleccionado
   ...
   # Seleccionar los nombres únicos de los oponentes
 ...
 output$selected_match_id <- renderText({
   ...
   # Buscar el partido correspondiente entre los dos equipos seleccionados según el tipo de partido (Ida o Vuelta)
   if (tipo_partido == "Ida") {
 ...
 calcular_prediccion <- function(equipo_local, equipo_visitante) {
   # Filtrar el dataframe df_aux según los equipos seleccionados
   #df_filtrado <- df_aux[df_aux$home_team == equipo_local & df_aux$visiting_team == equipo_visitante, ]
   
   # Modelo para sets del equipo local
   model_local <- glm(home_sets_won ~ home_team + visiting_team, 
   ...
   # Modelo para sets del equipo visitante
   model_visitante <- glm(visiting_sets_won ~ home_team + visiting_team, 
                          family = poisson(link = log), data = df_aux)
